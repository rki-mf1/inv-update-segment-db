...
segment_good_len_few_ambig <- segment_good_len[which(segment_good_len$V5 <= 5), ]
# remove sequences with wrong characters
segment_good_len_few_ambig_goodqual <- segment_good_len_few_ambig[which(segment_good_len_few_ambig$V4 <= 0), ]

# read in coresponding fasta file
fastain <- read.fasta(fasta)
fastaout <- fastain[c(which(names(fastain) %in% segment_good_len_few_ambig_goodqual$V1))]
